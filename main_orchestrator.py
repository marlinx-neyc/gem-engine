...
"""
================================================================================
GEM-V36D 龜山島海象氣-數值分析、周易易數同化與 SSOT 最高統合系統 (v36D.16.5)
================================================================================
專家角色定位：
        ...
        swarm_plan = [
            {
                ...
                "assigned_pier": "【南岸權宜碼頭】" if data.delta_theta_deg < 45.0 else "無 (雙岸失效)",
                "tactical_action": "授權靠泊南岸" if data.delta_theta_deg < 45.0 else "直航返航烏石港"
            }
        ]
...
# ==============================================================================
# 8. Gymnasium 10D 強化學習代理人與零延遲硬掩碼熔斷
# ==============================================================================
...
def train_rl_agent(policy_net: GuerrillaRLPolicyNet, episodes: int = 50):
...
class TGGuerrillaMasterEngine:
    ...
    def execute(self, telemetry: MarineSafetyData) -> Dict[str, Any]:
        ...
        if physics_res["has_veto"]:
            ...
            confidence_label = "🔴 0.0% [物理 VETO 熔斷]"
            berthing = "無 (雙岸失效，禁止靠泊)"
            evac = "【南岸權宜碼頭】 → 返航【烏石港】"
            morning_tactic = "⚠️ 上午游擊調撥：北岸越浪，08:30 班次改至【南岸權宜碼頭】靠泊或延滯登至 10:00"
            afternoon_tactic = "🚨 下午游擊撤退：預測午後 ESE 巽宮風陣，10:50/13:50 雙預警廣播，11:20 止登【南岸碼頭】，14:20 強制清島撤離至【烏石港】"
            summary = f"四層 Hard VETO 剛性熔斷（Hs={physics_res['hs_pier_m']}m）。執行「10:50/13:50 雙預警，11:20 止登【南岸碼頭】，14:20 全員撤離至【烏石港】」，預計撤離耗時 {evac_minutes} 分鐘。"
        else:
            ...
            summary = f"四層 Hard VETO 全數 PASS，八門對齊為【{qimen_res['gate_state']}】，安全執行游擊式動態調撥。"
        ...
        trend_forecast_3d = {
            "d_plus_1": {"date": "2026-09-22", "status": "🟢 PASS 全線開放", "hs_m": 0.95, "desc": "UKC/FB 全數 PASS，授權 08:30 首班靠泊【北岸碼頭】"},
            "d_plus_2": {"date": "2026-09-23", "status": "🟡 條件靠泊", "hs_m": 1.10, "desc": "Δθ=39.0°，T-30 移防【南岸權宜碼頭】"},
            "d_plus_3": {"date": "2026-09-24", "status": "🟢 PASS 全線開放", "hs_m": 0.85, "desc": "海象平穩，全線正常開放"}
        }
...
if __name__ == "__main__":
    raw_api_sample = {
        ...
        "s_cos_sim": 0.9421,
        "high_tide_time_str": "2026-09-21 09:12:00",
        "video_overtopping_rate": 0.0,
