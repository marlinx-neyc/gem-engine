...
* **警戒紅 (`#ef4444`)**：VETO 觸發、封島標籤與 Hs 警戒水平線 (1.20m)。
* **安全綠 (`#10b981`)**：放行狀態、PASS 徽章與 Level 5 算子外框。
* **高光黃 (`#facc15`)**：戰術裁決總結、Ground Truth 方案 C 評估與關鍵極限數值。
* **警戒橘 (`#f59e0b`)**：游擊調度卡片邊框、Wind 風速警戒線 (10.80m/s) 與時窗預警。
* **紫光 (`#c084fc`)**：奇門卡片邊框、64D 量子拓撲相干性與長浪週期線。
* **ECharts 互動與游標規範**：
  * 所有 ECharts 圖表 `splitLine` 統一設為 50% 透明度 (`rgba(51, 65, 85, 0.5)`)。
  * 圖表統一配置 `axisPointer: { type: 'cross' }` 追蹤線，懸浮 Tooltip 精確顯示時間、數值與當下戰術狀態。
  * 垂直時間判點虛線 (11:00 / 14:00) 必須關閉端點箭頭 (`symbol: 'none'`)。
...
* 整合標題列（含 `● LIVE SSOT` 動態藍呼吸燈與連線計數）與升格 AI 雙核指標區（AI 信心度 100.0%、σ 方差 0.3125、精確度提升 +58.4% 與微觀/宏觀雙色動態權重條）。
* 最上層橫向併入 **🚀 Level 5 邊緣視覺、水動力與賽局高維整合算子** 區塊，內含 4 大算子子卡片：
  1. CV 視覺越浪率 (p/min) `#facc15`
  2. MMSI 客輪沉降 (Squat m) `#38bdf8`
  3. FNO 100ms 預報波高 (m) `#ef4444`
  4. 64D 量子拓撲相干性 `#c084fc`
* 附帶 Swarm 賽局排隊狀態與高維相干收斂宣告。

### 2. 左側戰術控制欄 (32% 欄寬) 共 4 張卡片
1. **PINN Engine 即時水文門檻比對**：Hs Pier, W Local, UKC, FB Pier 與 PASS/VETO 徽章。
2. **當日船班官模燈號與時窗比對**：官方 vs GEM 燈號，並列出 08:30 首班、11:20 止登與 14:20 強制撤離細節。
...
### 3. 右側動態圖表陣列 (68% 欄寬)
* **2x2 時間線圖表**：Hs (含 1.20m 黃色警戒線)、Wind (含 10.80m/s 橘色警戒線)、UKC、Tp 週期，注入 11:00 與 14:00 無箭頭判點虛線。
* **未來七天雙軸趨勢圖**：Hs 與 Wind 雙 Y 軸曲線。
* **戰略三方案評估 (Ground Truth Comparison)**：方案 A、B、C 據值對比（方案 C 採高光黃 `#facc15`），底部帶有高光黃【戰略方案總結】宣告列。
