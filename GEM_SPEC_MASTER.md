...
本規範為 GEM-V36D 數位雙生海事戰術控制台（包含 Desktop 端、Mobile 端與 Python 主控算子）之最高技術標準。全系統維運與程式碼變更必須 100% 恪守「版面幾何固定化 (CLS = 0)」與「數據單向更新化 (Data-Only SSOT)」雙柱原則。
...
* **ECharts 互動與止/撤虛線色彩對齊規範**：
  * **止/撤虛線色彩 100% 匹配該圖折線**：Hs 圖止撤虛線採紅色 (`#ef4444`)；Wind 圖採橘色 (`#f59e0b`)；UKC 圖採綠色 (`#10b981`)；Tp 圖採紫色 (`#c084fc`)。
...
* **半小時預警機制與碼頭標註**：
  * 止登 (11:20) 與撤離 (14:20) 前 0.5 小時需加入預發警報（**10:50 預發止登警報** / **13:50 預發撤離警報**）。
  * 登島靠泊與撤退離島必須明確標註實體碼頭名稱（如【南岸權宜碼頭】與撤離目的地【烏石港】）。
...
2. **當日船班官模燈號、預警時窗與戰略方案總結整合卡片**：官方 vs GEM 燈號、08:30 首班登島靠泊【南岸權宜碼頭】、10:50/13:50 半小時預警、11:20 止登與 14:20 強制撤離至【烏石港】細節，並**直接嵌入高光黃【戰略方案總結】**。
3. **⚓ 游擊式動態調度與時窗預判 (`#f59e0b` 邊框)**：登島靠泊【南岸權宜碼頭】與撤退離島至【烏石港】。
4. **🌀 颱風、雙週長浪、潮汐與奇門氣場預判專區 (`#a855f7` 邊框)**：中颱/955hPa 動態、250km 暴風半徑、2D 氣壓梯度 (1.10 hPa/km)、朔望大潮 (高潮 +1.20m / 低潮 -0.40m)、東北季風共振、Week 1/2 雙週長浪趨勢、&plusmn;15% 不確定性與奇門 100% 匹配提示。

### 3. 右側動態圖表陣列 (68% 欄寬)
* **2x2 時間線圖表**：Hs (黃虛線 1.20m)、Wind (橘虛線 10.80m/s)、UKC (紅虛線 1.50m)、Tp 週期 (紫虛線 12.0s)，11:00 與 14:00 止撤虛線色彩與該圖折線 100% 對齊且關閉箭頭。
...
6. **當日船班官模燈號、預警時窗與未來 3 天開/封島預測卡片**：包含 10:50/13:50 預警、08:30/11:20/14:20 船班與碼頭細節，以及 D+1 (🔴 封島), D+2 (🟡 限制靠泊), D+3 (🟢 全線開放) 預測模組。
7. ⚓ 游擊式動態調度處置卡片 (`#f59e0b` 邊框)。
8. 🌀 颱風、長浪與潮汐氣場預判卡片 (`#a855f7` 邊框)。
...
```json
{
  ...
  "level5_advanced_metrics": {
    ...
    "swarm_dispatch_plan": [
      { "agent_id": 1, "vessel_label": "凱鯨號 (Agent 1)", "assigned_pier": "【南岸權宜碼頭】", "tactical_action": "直航返航烏石港" }
    ]
  },
